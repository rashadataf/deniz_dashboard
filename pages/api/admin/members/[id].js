...
apiRoute.get(async (req, res) => {
  ...
  try {
    const { client, db } = await connectToDatabase();
    const member = db.collection("members").find({ _id: id });
...
apiRoute.put(async (req, res) => {
  ...
  try {
    ...
    const member = await members.findOne({ _id: new mongodb.ObjectID(id) });
    if (member.imageUrl) {
      if (imageUrl) {
        if (imageUrl !== member.imageUrl) {
          fs.unlinkSync(path.join(process.cwd(), member.imageUrl));
        }
      } else {
        fs.unlinkSync(path.join(process.cwd(), member.imageUrl));
      }
...
apiRoute.delete(async (req, res) => {
  ...
  const adminEmail = session.member.email;
  const { client, db } = await connectToDatabase();
  const admins = db.collection("admins");
  const admin = await admins.findOne({ email: adminEmail });
  if (!admin) {
  ...
  try {
    ...
    let image = deletedMember.value.imageUrl;
    if (image && image !== "null") {
